document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".input-form-title").forEach(title => {
        title.addEventListener("input", function(){
            ...
            document.title = `${this.value} - Data form`
    ...
    const addDistrict = () => {
        document.querySelectorAll(".add-district").forEach(question =>{
            question.addEventListener("click", function(){
                fetch('add_district', {
                    method: "POST",
                    headers: {'X-CSRFToken': csrf},
                        ...
                        if(choicesElement.dataset.id === this.dataset.id){
                            if(this.value === "multiple choice" || this.value === "checkbox"){
                                ...
                                if(this.value === "short"){
                                    ...
                                    let ele = document.createElement("div");
                                    ele.innerHTML = `<div class="answers" data-id="${this.dataset.id}">
                                    <textarea class="long-answer" disabled placeholder="Long answerss text ${this.dataset.id}" ></textarea>
                                </div>`
                                    this.parentNode.insertBefore(ele, this.parentNode.childNodes[4])
                                }
                    ...
                    document.querySelectorAll(".question-box").forEach(question => {
                        ...
                        if((this.value === "multiple choice" || this.value === "checkbox") && question.dataset.id === this.dataset.id){
                            ...
                            if(this.value === "short"){
                                ...
                                let ele = document.createElement("div");
                                ele.innerHTML = `<div class="add-district" data-id="${this.dataset.id}">
                                ...
                                this.parentNode.insertBefore(ele, this.parentNode.childNodes[4])
                            }else if(this.value === "sectors"){
                                let ele = document.createElement("div");
                                ele.innerHTML = `<div class="answers" data-id="${this.dataset.id}">
                                // <textarea class="long-answer" disabled placeholder="Select Sectors as per Block" ></textarea>
                            </div>`
                                this.parentNode.insertBefore(ele, this.parentNode.childNodes[4])
